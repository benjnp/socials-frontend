...
const PinDetail = ({ user }) => {
  ...
  return (
    ...
    <div className="flex xl:flex-row flex-col m-auto bg-white" style={{maxWidth: '1500px', borderRadius: '32px'}}>
      <div className="flex justify-center items-center md:items-start flex-initial xl:w-3/5">
      ...
      <div className="w-full p-5 flex-1 xl:min-2-620">
        ...
        <Link to={`/user-profile/${pinDetail.postedBy?._id}`} className="flex gap-2 mt-5 items-center bg-white rounded-lg">
            ...
            />
            <p className="font-base capitalize">{pinDetail.postedBy?.userName}</p>
        </Link>
        ...
        <div className="max-h-370 overflow-y-auto">
          {pinDetail?.comments?.map((comment, i) => (
            <div className="flex gap-2 mt-5 items-center bg-white rounded-lg"  key={i}>
              ...
              <img src={comment.postedBy.image} alt="commenter-image"
        ...
        { user && (<div className="flex flex-wrap mt-6 gap-3 items-center">
          <Link to={`/user-profile/${pinDetail.postedBy?._id}`}>
              <img src={pinDetail.postedBy?.image} alt="user-profile" 
                  className="w-8 h-8 rounded-full cursor-pointer"
              />
          ...
          <button
            ...
            onClick={addComment}
          >
            {isAddingComment ? "Adding the comment..." : "Post Comment"}
          </button>
        </div> )}
      </div>
    </div>
